...
public class ValidatorY implements Validator {
    ...
    public void validate(FacesContext facesContext, UIComponent uiComponent, Object o) throws ValidatorException {
        ...
        if (o == null) {
            throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, null, "The Y field cannot be empty!"));
        }
        ...
        if ("button".equals(source)) {
            ...
            if (yValue.compareTo(MIN_Y) <= 0 || yValue.compareTo(MAX_Y) >= 0) {
                throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, null, String.format("The Y value must be in the range (%s;%s)", MIN_Y, MAX_Y)));
            }
        }
    }
}